  ...
  >
    <div class="group h-4/5">
      <router-link :to="{name: 'Channel', params: { id: comment._id }}">
        <p class="text-lg text-gray-500 line-clamp-4 bg-gray-100
        rounded-lg border border-gray-200 text-left pl-2">
          {{ comment.text }}
        </p>
      </router-link>
    </div>
    <div class="p-2 pt-0 flex flex-1 justify-between align-center h-1/5">
      <div
        @click="switchModal()"
        class="subcomment flex
        justify-around max-w-min text-yellow-400 hover:text-yellow-500"
      >
        {{ getSubcomments ? getSubcomments.length : 0 }} <AnnotationIcon class="h-6 w-6 ml-2" />
      </div>
